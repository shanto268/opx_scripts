...
pi_half_len = 36 # needs to be a multiple of 4
...
config = {
    ...
    "controllers": {
        "con1": {
            "type": "opx1",
            "analog_outputs": {
                1: {"offset": -0.0228},  # qubit I
                2: {"offset": 0.0095},  # qubit Q
                3: {"offset": -0.015},  # rr I
                4: {"offset": -0.002},  # rr Q
            },
            "digital_outputs": {},
            "analog_inputs": {
                # 1: {"offset": -0.00022577816162108744},  # rr I
                # 2: {"offset": -0.07324522040405274}  # rr Q
                1: {"offset": -0.09258526593889509,"gain_db": 2},  # rr I
                2: {"offset": -0.09859225603376115,"gain_db": 2}  # rr Q
            },
        },
    },
    "elements": {
        ...
        "rr": {
            ...
            "time_of_flight": 284, #without smearing # should be multiple of 4
    ...
    "integration_weights": {
        "integW1_cos": {
            "cosine": [np.cos(b)] * rr_pulse_len_in_clk,
            "sine": [-np.sin(b)] * rr_pulse_len_in_clk,
        },
        "integW1_sin": {
            "cosine": [np.sin(b)] * rr_pulse_len_in_clk,
            "sine": [np.cos(b)] * rr_pulse_len_in_clk,
        },
        "integW1_minus_sin": {
            "cosine": [-np.sin(b)] * rr_pulse_len_in_clk,
            "sine": [-np.cos(b)] * rr_pulse_len_in_clk,
    ...
    "mixers": {
        "mixer_q1": [{"intermediate_frequency": qb_IF, "lo_frequency": qb_LO, "correction": IQ_imbalance(0.015,-0.095)}],
        "mixer_rl1": [{"intermediate_frequency": rr_IF, "lo_frequency": rr_LO, "correction": IQ_imbalance(-0.04,-0.065)}],
    }
}
